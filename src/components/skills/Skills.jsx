...
import { SKILLS_DEV, SKILLS_HACKER } from '../../constants.js';

export function Skills({ isHackerMode }) {
  const tools = isHackerMode ? SKILLS_HACKER : SKILLS_DEV;
        ...
        {tools.map((tool, index) => (
          <div
            ...
            className={`p-4 border rounded bg-slate-950/30 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 ${
              isHackerMode
                ? 'border-emerald-500/10 hover:border-emerald-500/40 text-emerald-400/90'
